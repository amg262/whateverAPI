...
public class Validator : Validator<CreateJokeRequest>
{
    public Validator()
    {
        ...
        RuleFor(x => x.Type).IsInEnum().WithMessage("Invalid joke type");
        RuleFor(x => x.Tags).Must(tags => tags?.Count <= 10)
            .WithMessage("Maximum 5 tags allowed").When(x => x.Tags != null);
        RuleForEach(x => x.Tags).MaximumLength(20)
            .WithMessage("Tag length cannot exceed 20 characters").When(x => x.Tags != null);
    }
}
...
public class CreateJoke : Endpoint<CreateJokeRequest, JokeResponse>
{
    ...
    public override async Task HandleAsync(CreateJokeRequest req, CancellationToken ct)
    {
        ...
        JokeResponse response = EntityMapper.JokeToJokeResponse(joke);
